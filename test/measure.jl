@testset "Measure" begin
    ...
    @test monomials(μ) == monomial_vector([x^3 * y, x^2 * y^2, x^2 * y, x * y^2])
    @test monomial.(moments(μ)) == monomial_vector([x^3 * y, x^2 * y^2, x^2 * y, x * y^2])
    ...
    err = ErrorException("The monomial `x` occurs twice with different values: `1` and `2`")
