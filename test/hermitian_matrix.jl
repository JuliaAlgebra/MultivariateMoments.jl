...
@testset "VectorizedHermitianMatrix" begin
    ...
    @test square_getindex(Q, 1:2) isa
          VectorizedHermitianMatrix{Int,Bool,Complex{Int}}
    @test square_getindex(Q, 1:2).Q == [1, 2, 3, -1]
    @test square_getindex(Q, 2:-1:1) isa
          VectorizedHermitianMatrix{Int,Bool,Complex{Int}}
    @test square_getindex(Q, 2:-1:1).Q == [3, 2, 1, -1]
    @test square_getindex(Q, 1:1) isa
          VectorizedHermitianMatrix{Int,Bool,Complex{Int}}
    @test square_getindex(Q, 1:1).Q == [1]
    @test square_getindex(Q, 2:2) isa
          VectorizedHermitianMatrix{Int,Bool,Complex{Int}}
    ...
    N = MultivariateMoments.vectorized_hermitian_matrix(
        Int,
        (i, j) -> M[i, j],
        3,
        3:-1:1,
